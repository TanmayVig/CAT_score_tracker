import { BarChart3, Check, ChevronDown, ChevronRight, Edit3, Plus, RefreshCw, Save, X } from "lucide-react";
...
import { createMock, fetchMocks, updateMock } from "./api";
import { CAT_SECTIONS, GAP_TOPICS, createEmptyMock, type CatSection, type MockAttempt, type MockAttemptInput } from "./shared/cat";
...
function cloneMockInput(mock: MockAttemptInput): MockAttemptInput {
  return {
    ...
    sections: {
      VARC: { ...mock.sections.VARC, gaps: [...mock.sections.VARC.gaps], learnings: [...mock.sections.VARC.learnings] },
      DILR: { ...mock.sections.DILR, gaps: [...mock.sections.DILR.gaps], learnings: [...mock.sections.DILR.learnings] },
      QA: { ...mock.sections.QA, gaps: [...mock.sections.QA.gaps], learnings: [...mock.sections.QA.learnings] }
    }
  };
}
...
function App() {
  ...
  const sortedForCharts = useMemo(
    () => [...mocks].sort((first, second) => new Date(first.createdAt).getTime() - new Date(second.createdAt).getTime()),
    [mocks]
  );
  ...
  function toggleGap(section: CatSection, topic: string) {
    ...
    const nextGaps = gaps.includes(topic) ? gaps.filter((gap) => gap !== topic) : [...gaps, topic];
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <button className="ghost-button" type="button" onClick={() => void loadMocks()} disabled={loading}>
      ...
      <section className="layout-grid">
        <form className="panel form-panel" onSubmit={(event) => void submitMock(event)}>
          <div className="panel-heading">
            ...
            {editingId ? (
              <button className="icon-button" type="button" onClick={resetForm} aria-label="Cancel edit" title="Cancel edit">
          ...
          <div className="field-grid">
            <label>
              Mock name
              <input value={draft.name} onChange={(event) => updateField("name", event.target.value)} placeholder="AIMCAT 2501" required />
            </label>
            <label>
              Total marks
              <input
                ...
                onChange={(event) => updateField("totalMarks", Number(event.target.value))}
                required
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={(event) => updateField("percentile", Number(event.target.value))}
                required
              />
            </label>
          </div>
              ...
              <section className="section-box" key={section}>
                ...
                <div className="field-grid compact">
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) => updateSection(section, { marks: Number(event.target.value) })}
                      required
                    />
                  </label>
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) => updateSection(section, { percentile: Number(event.target.value) })}
                      required
                    />
                  </label>
                </div>
                ...
                <div className="topic-group" aria-label={`${section} gaps`}>
                  ...
                  {GAP_TOPICS[section].map((topic) => (
                    <button
                      className={draft.sections[section].gaps.includes(topic) ? "topic-chip selected" : "topic-chip"}
                      type="button"
                      key={topic}
                      onClick={() => toggleGap(section, topic)}
                    >
                      {draft.sections[section].gaps.includes(topic) ? <Check aria-hidden="true" size={14} /> : null}
                      {topic}
                    </button>
                  ))}
                </div>
                ...
                <div className="learning-list">
                  {draft.sections[section].learnings.map((learning, index) => (
                    <div className="learning-row" key={`${section}-${index}`}>
                      <input
                        value={learning}
                        onChange={(event) => updateLearning(section, index, event.target.value)}
                        placeholder={`${section} learning`}
                      />
                      <button className="icon-button" type="button" onClick={() => removeLearning(section, index)} aria-label="Remove learning" title="Remove learning">
                        <X aria-hidden="true" size={16} />
                      </button>
                    </div>
                  ))}
                  <button className="ghost-button small" type="button" onClick={() => addLearning(section)}>
            ...
            {mocks.map((mock) => {
              ...
              <article className="mock-item" key={mock.id}>
                ...
                <div className="mock-actions">
                  ...
                  <button className="icon-button" type="button" onClick={() => editMock(mock)} aria-label={`Edit ${mock.name}`} title="Edit mock">
        ...
        <article className="panel chart-panel">
          <div className="panel-heading">
            ...
            <div className="segmented-control" aria-label="Section metric">
              <button className={metricMode === "percentile" ? "active" : ""} type="button" onClick={() => setMetricMode("percentile")}>
                Percentile
              </button>
              <button className={metricMode === "marks" ? "active" : ""} type="button" onClick={() => setMetricMode("marks")}>
                Marks
              </button>
            </div>
          </div>
